 ...
 * Multi-layered cortex mesh with gyri/sulci noise, subsurface glow,
 * white-matter tracts, electrical impulses, and UnrealBloom post-processing.
 */
export function NeuralBrainCanvas({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    let linksDone = false;
    for (let i = 0; i < TOTAL_NODES && !linksDone; i++) {
    ...
    function animate() {
      ...
      // Brain group rotation — slow auto-rotate + mouse influence
      const targetRotY = elapsed * 0.06 + mouseCurrent.x * 0.4;
      const targetRotX = -0.04 + mouseCurrent.y * 0.2;
    ...
    return () => {
      ...
      if (mount) mount.removeEventListener('pointermove', onPointerMove);
