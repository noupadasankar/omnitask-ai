...
# so the next step resumes on the same live page with the same DOM state.
...
async def run_job(job: dict, publisher: EventPublisher, pw) -> None:
    ...
    if prev_live is not None and prev_live.page is not None:
        try:
            # Reuse the existing live page — don't reinitialize the browser.
            live = prev_live
            page = live.page
            await publisher.publish(session_id, "worker:browser_state", {"state": "RUNNING"})
        except Exception:
            # Stale session — fall through to fresh init below.
            _session_pages.pop(session_id, None)
            prev_live = None
            live = None

    if live is None:
        # Fresh browser init path (first step or whole-plan dispatch).
        ...
        try:
            ...
            await publisher.publish(session_id, "execution:failed", {
                "sessionId": session_id, "reason": "browser_init_error", "message": str(init_err),
            })
    ...
    try:
        ...
        if keep_session:
            ...
            # on the same page (same DOM state, same URL, same logged-in context).
            _session_pages[session_id] = live
        else:
            # Whole-plan or final step — clean up.
            _session_pages.pop(session_id, None)
        ...
        if not keep_session:
            ...
            active_ctl = input_ctl or (live.input_ctl if live is not None else None)
            active_streamer = streamer or (live.streamer if live is not None else None)
            if active_ctl is not None:
                await active_ctl.stop()
            if active_streamer is not None:
                await active_streamer.stop()
...
_AGENT_LABEL = {
    ...
    "FORM_AGENT": "Form Agent",
    "VERIFIER_AGENT": "Verifier Agent",
    "ORCHESTRATOR": "Orchestrator",
}


async def _emit_agent_status(publisher, session_id, step, screenshot_b64=None, step_output=None):
    """Emit the agent_status event shown live in the dashboard."""
    agent = step.get("agent") or "BROWSER_AGENT"
    human_text = (
        step.get("emit_status")
        or step.get("description")
        or f"Executing step {step.get('index', 0) + 1}"
    )
    await publisher.publish(session_id, "agent_status", {
        "sessionId": session_id,
        "timestamp": now_ms(),
        "agent": agent,
        "agentLabel": _AGENT_LABEL.get(agent, agent),
        "stepIndex": step.get("index"),
        "status": human_text,
        "emit_status": human_text,
...
async def _execute_as_vision_agent(page, step, publisher, session_id):
    """VISION_AGENT: takes a screenshot for visual analysis."""
    shot = await _safe_screenshot(page)
    await publisher.publish(session_id, "vision:analysis_started", {
        "sessionId": session_id, "stepIndex": step.get("index"),
    })
    return {"screenshot": shot, "agent": "VISION_AGENT", "url": page.url}


async def _execute_as_data_agent(page, step, publisher, session_id):
    """DATA_AGENT: extracts text or structured data from the page."""
    action = step.get("action", "extract_text")
    return await execute_action(page, step, publisher, session_id)


async def _execute_as_form_agent(page, step, publisher, session_id):
    """FORM_AGENT: form-fill actions (type, select, upload_file) with extra care."""
    return await execute_action(page, step, publisher, session_id)


async def _execute_as_verifier_agent(page, step, publisher, session_id):
    """VERIFIER_AGENT: validates expected state after a previous step."""
    shot = await _safe_screenshot(page)
    result = await execute_action(page, step, publisher, session_id)
    await publisher.publish(session_id, "execution:verified", {
        "sessionId": session_id, "stepIndex": step.get("index"),
        "screenshot": shot, "result": result,
        "passed": result.get("success", True) if isinstance(result, dict) else True,
    })
    return result


async def _dispatch_to_agent(page, step, publisher, session_id):
    """Route a step to the correct specialist agent based on step.agent field."""
    agent = step.get("agent") or "BROWSER_AGENT"
    if agent == "VISION_AGENT":
        return await _execute_as_vision_agent(page, step, publisher, session_id)
    if agent == "DATA_AGENT":
        return await _execute_as_data_agent(page, step, publisher, session_id)
    if agent == "FORM_AGENT":
        return await _execute_as_form_agent(page, step, publisher, session_id)
    if agent == "VERIFIER_AGENT":
        return await _execute_as_verifier_agent(page, step, publisher, session_id)
    # Default: BROWSER_AGENT or ORCHESTRATOR
    return await execute_action(page, step, publisher, session_id)


async def _run_steps(page, steps, publisher, session_id, live=None):
    """Execute a Node-generated step plan.

    Each step is routed to the correct specialist agent (BROWSER_AGENT,
