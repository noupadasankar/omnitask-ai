    ...
    async def apply_to_job(self, job: Dict) -> bool:
        ...
        try:
            ...
            for step in range(max_steps):
                ...
                # Detect CAPTCHA / OTP / custom-dropdown before trying to autofill.
                # Give the user up to 90 s to resolve in the live browser view.
                blocker = await self._detect_blocker(modal) or await self._detect_blocker(page)
                if blocker:
                    resolved = await self._handle_blocker(blocker, job, page)
                    if not resolved:
                        await self._dismiss_modal(page)
                        return False
                    # Re-fetch modal handle — DOM may have refreshed after resolution.
                ...
                if await self._modal_signature(modal) == sig_before:
                    ...
                    try:
                        ...
                        pass
                    if await self._modal_signature(modal) == sig_before:
                        # Still stuck — check if a blocker appeared (CAPTCHA / OTP
                        # / custom dropdown) and give the user a chance to fix it.
                        blocker = (await self._detect_blocker(modal)
                                   or await self._detect_blocker(page))
                        if blocker:
                            resolved = await self._handle_blocker(blocker, job, page)
                            if resolved:
                                continue   # retry this step now that blocker is gone
                        self.logger.warning(
