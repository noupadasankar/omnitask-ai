...
export async function getUserHistory(): Promise<any[]> {
  const { data } = await api.get<{ data: any[]; nextCursor: string | null; hasMore: boolean }>('/agent/history');
  return data.data ?? [];
}

export async function getUserMemories(): Promise<any[]> {
  const { data } = await api.get<{ data: any[]; nextCursor: string | null; hasMore: boolean }>('/agent/memory');
  return data.data ?? [];
}
