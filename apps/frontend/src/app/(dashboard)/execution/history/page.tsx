...
import { getUserHistory, startGoalExecution } from '@/services/agent.service';
...
export default function HistoryPage() {
  ...
  const handleRerun = async (goal: string, runId: string) => {
    if (!goal || rerunning) return;
    setRerunning(runId);
    try {
      const { sessionId } = await startGoalExecution({ goal, mode: 'autonomous', profile: 'balanced' });
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-4 mt-4">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleRerun(goal, run.id)}
                        disabled={rerunning === run.id}
                        className="h-8 px-3 rounded-lg border border-white/10 bg-white/[0.03] text-[10px] font-bold text-zinc-400 hover:text-white hover:border-white/20 transition-all flex items-center gap-1.5 disabled:opacity-40"
                      >
                        <RotateCcw className={`h-3 w-3 ${rerunning === run.id ? 'animate-spin' : ''}`} />
                        RERUN
                      </button>
