...
export default function DashboardPage() {
  ...
  const QUICK_PROMPTS = [
    ...
    { icon: '🛒', label: 'Price Compare', prompt: 'Compare iPhone 16 Pro prices on Amazon vs Flipkart and show a comparison table' },
    { icon: '📰', label: 'News Digest', prompt: 'Find latest technology news headlines and create a summary digest of top stories' },
    { icon: '💼', label: 'LinkedIn Jobs', prompt: 'Apply to senior React developer jobs on LinkedIn in Bangalore' },
    { icon: '🍕', label: 'Order Food', prompt: 'Order chicken biryani under ₹250 on Swiggy or Zomato' },
            ...
            >
              {QUICK_PROMPTS.map((qp, i) => (
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3.5 py-2 text-xs text-zinc-400 hover:bg-white/[0.05] hover:text-white transition-all"
                >
                  <span>{qp.icon}</span>
                  {qp.label}
                </button>
