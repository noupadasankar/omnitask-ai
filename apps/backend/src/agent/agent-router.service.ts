...
export class AgentRouterService {
  ...
  async route(
    ...
    const prefsCacheKey = `prefs:${context.userId}`;
    let learnedPrefs = await this.cache.get<UserDomainPreferences>(prefsCacheKey);
    if (!learnedPrefs) {
      learnedPrefs = await this.preferenceMemory.getPreferences(context.userId);
      await this.cache.set(prefsCacheKey, learnedPrefs, 600_000);
    }
    ...
    if (agent) {
      ...
      const sitesCacheKey = `rankedSites:${context.userId}:${agent.category}`;
      let learnedSites = await this.cache.get<string[]>(sitesCacheKey);
      if (!learnedSites) {
        const raw = await this.learningEngine.getRankedSites(context.userId, agent.category);
        learnedSites = raw.map((site: { site: string }) => site.site);
        await this.cache.set(sitesCacheKey, learnedSites, 600_000);
      }
