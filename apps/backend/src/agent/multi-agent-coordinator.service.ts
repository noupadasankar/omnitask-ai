...
/**
 * @responsibility Parallel multi-agent session lifecycle: spawn, coordinate, collect results.
 * Does NOT parse goals or create plans (OrchestratorPipelineService owns that).
 * Does NOT manage long-running supervisor trees (SupervisorOrchestratorService owns that).
 */
@Injectable()
  ...
  async handleNaturalLanguageControl(
    ...
    switch (parsedCommand.type) {
      ...
      case 'provide_data':
        ...
        // Delivered exclusively via an in-process channel — never broadcast over
        // WebSocket. The UI receives a data-received acknowledgement without the
        // sensitive payload.
        dbStatus = ExecutionSessionStatus.RUNNING;
        this.executionEngine.injectUserData(sessionId, parsedCommand.payload?.data);
        this.wsGateway.emitToSession(sessionId, 'execution:event', {
