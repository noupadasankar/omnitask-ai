...
const push = spawnSync(
  ...
  isWin
    ? ["/d", "/s", "/c", winShimArgs("pnpm", ["--filter", "backend", "prisma", "db", "push"])]
    : ["pnpm", "--filter", "backend", "prisma", "db", "push"],
  { cwd: repoRoot, stdio: "inherit", env: { ...process.env, NODE_OPTIONS: "" } },
);
if (push.status !== 0) {
  console.warn("⚠ prisma db push failed — schema may not be up to date. Continuing anyway.");
}
...
if (!nodeOptions.some((option) => option.startsWith("--max-old-space-size="))) {
  nodeOptions.push("--max-old-space-size=4096");
}
